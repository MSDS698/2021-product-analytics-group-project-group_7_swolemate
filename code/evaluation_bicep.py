...
class Part:
    ...
    def dist(part1, part2):
        return np.sqrt(
            np.square(part1.x - part2.x) + np.square(part1.y - part2.y))


class Pose:

    # Fix this part names array based on order of pose estimation coordinates
    PART_NAMES = ['nose', 'neck',  'right_shoulder',
                  'right_elbow', 'right_wrist', 'left_shoulder',
                  'left_elbow', 'left_wrist', 'right_hip', 'right_knee',
    ...
    def __str__(self):
        ...
        for name in self.PART_NAMES:
            _ = "{}: {},{}".format(
                name, getattr(self, name).x, getattr(self, name).y)
            out = out + _ + "\n"
        return out
    ...
    def print(self, parts):
        out = ""
        for name in parts:
            if not in name self.PART_NAMES:
...
class PoseSequence:
    ...
    """
    def __init__(self, sequence, bool_v=False):
        # sequence of poses where a pose is a coordinate for each part
        self.poses = []
        ...
        """
        to generalize our application to account for users with
        different body length measurements, distance
        ...
        right and left hip keypoints. This normalization works extremely well:
        we observe that the torso length stays very
        ...
        """
        torso_lengths = np.array([Part.dist(pose.neck, pose.left_hip)
                                  for pose
                                  in self.poses
                                  if pose.neck.exists
                                  and pose.left_hip.exists] +
                                 [Part.dist(pose.neck, pose.right_hip)
                                  for pose
                                  in self.poses
                                  if pose.neck.exists
                                  and pose.right_hip.exists])
        ...
        for pose in self.poses:
            for attr, part in pose:
                setattr(pose,
                        attr,
                        part / mean_torso)  # the certain attribute of the pose
...
def load_features(names, data=None, side=None, bool_val=False):
    ...
    Side --> encoded as left or right from
    user uploaded videos (without confidence encoded)
    """
    ...
    for filename in names:
        if not bool_val:
            ...
            ps = PoseSequence(data, bool_v=True)
        poses = ps.poses

        """
        For certain exercises, we resolve the ambiguity in camera perspective.
        For example, the bicep curl exercise is
        ...
        throughout all frames of the exercise.
        This accurately detects perspective all of our input videos
        """
        if not bool_val:
            right_present = [1 for pose in poses
                             if pose.right_shoulder.exists and
                             pose.right_elbow.exists and
                             pose.right_wrist.exists]  # chk right poses exist
            left_present = [1 for pose in poses
                            if pose.left_shoulder.exists and
                            pose.left_elbow.exists and
                            pose.left_wrist.exists]  # chk left poses exist
            right_count = sum(right_present)
            left_count = sum(left_present)
            side = 'right' if right_count > left_count else 'left'

        if side == 'right':
            joints = [(pose.right_shoulder,
                       pose.right_elbow,
                       pose.right_wrist,
                       pose.right_hip,
                       pose.neck)
        ...
        joints = [joint
                  for joint
                  in joints
                  if all(part.exists for part in joint)]

        # Shoulder to elbow (upper arm)
        upper_arm_vecs = np.array(
            [(joint[0].x - joint[1].x, joint[0].y - joint[1].y)
             for joint in joints])
        # Neck to hip (torso)
        torso_vecs = np.array(
            [(joint[4].x - joint[3].x, joint[4].y - joint[3].y)
             for joint in joints])
        # Elbow to wrist (forearm)
        forearm_vecs = np.array(
            [(joint[2].x - joint[1].x, joint[2].y - joint[1].y)
             for joint in joints])

        # normalize portion: SAME FOR ALL EXERCISES
        upper_arm_vecs = upper_arm_vecs / \
            np.expand_dims(np.linalg.norm(upper_arm_vecs, axis=1), axis=1)
        torso_vecs = torso_vecs / \
            np.expand_dims(np.linalg.norm(torso_vecs, axis=1), axis=1)
        forearm_vecs = forearm_vecs / \
            np.expand_dims(np.linalg.norm(forearm_vecs, axis=1), axis=1)
        ...
        A drawback with DTW is that it’s not robust to noise. When
            ...
            a keypoint sequence through a size 5 median filter twice
            before computing the DTW measures
        """
        upper_arm_torso_angle = np.degrees(
            np.arccos(
                np.clip(
                    np.sum(
                        np.multiply(
                            upper_arm_vecs, torso_vecs), axis=1), -1.0, 1.0)))
        upper_arm_torso_angle_filtered = medfilt(
            medfilt(upper_arm_torso_angle, 5), 5)  # size 5 median filter twice
        ...
        upper_arm_forearm_angle = np.degrees(
            np.arccos(
                np.clip(
                    np.sum(
                        np.multiply(
                            upper_arm_vecs, forearm_vecs),
                        axis=1), -1.0, 1.0)))
        upper_arm_forearm_angle_filtered = medfilt(
            medfilt(upper_arm_forearm_angle, 5),
            5)  # size 5 median filter twice
...
def Kmeans_test(names, X_train_1, X_train_2, y_train,
                data=None, side=None, bool_val=False, n_neighbors=3):
    if not bool_val:
    ...
    for example in range(len(names)):
        ...
        good_score = np.mean(f1_good[:n_neighbors]) + \
            np.mean(f2_good[:n_neighbors])
        bad_score = np.mean(f1_bad[:n_neighbors]) + \
            np.mean(f2_bad[:n_neighbors])

        percentage = (good_score - bad_score) / \
            bad_score * 100 * -1  # percentage

        """if percentage > 0:
            print(f"How good the form is: {round(percentage, 2)}%")
        else:
            print(f"How bad the form is: {round(percentage, 2)}%")"""
...
"""if __name__ == "__main__":
    ...
    # adding new data
    inp_seq_1, inp_seq_2 = load_features(['demo'],
                                         new_data,
                                         side,
                                         bool_val=True)
