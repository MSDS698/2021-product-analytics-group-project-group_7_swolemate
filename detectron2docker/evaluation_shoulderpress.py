...
if __name__ == "__main__":
    ...
    X_train_1, X_train_2 = load_features(X_train_names, exercise=exercise)
    X_test_1, X_test_2 = load_features(X_test_names, exercise=exercise)

    # adding new data
    inp_seq_1, inp_seq_2 = load_features(["demo"], new_data, "left", bool_val=True, exercise=exercise)
    X_train_1.append(inp_seq_1[0])
    X_train_2.append(inp_seq_2[0])
    y_train = np.append(y_train, 1)

    print(kmeans_test(["demo"], X_train_1, X_train_2, y_train, data, side, True, exercise=exercise))
